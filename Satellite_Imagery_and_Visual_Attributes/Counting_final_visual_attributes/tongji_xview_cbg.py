...
#    if 1:
        ...
        #object_detection_list = glob.glob(new_folder_list[i]+'/labels/*.txt')
            ...
            #     continue
            if os.path.exists('../final_obd_cbg_xview/xview_'+city_+'_'+str(year)+'.csv'):
                continue
            print(city, year)
            ...
            for nnn in range(7,0,-1):
                if os.path.exists(Dir+'city_year212223/dataset_xview_'+str(year)+'_'+city+str(nnn)):
                    label_dir = Dir+'city_year212223/dataset_xview_'+str(year)+'_'+city+str(nnn)
                    label_flg = 1
                    break

            if label_flg != 1:
                label_dir = Dir+'city_year212223/dataset_xview_'+str(year)+'_'+city_
            ...
            object_cnt = np.zeros((len(zl15_list),10))
        ...
        #col_names = DOTA_cols + [x+'_conf' for x in DOTA_cols]
           # pd_dict2.columns = DOTA_cols#names
            ...
            print(len(pd_dict))
            pd_dict.to_csv('../final_obd_cbg_xview/xview_'+city+'_'+str(year)+'.csv', index = False)
        #print(str(cnt)+' done')
