...
#    if 1:
        ...
        #object_detection_list = glob.glob(new_folder_list[i]+'/labels/*.txt')
            ...
            #     continue
            if os.path.exists('../final_obd_city_xview/xview_'+city+'_'+str(year)+'.csv'):
                continue
            print(city, year)
            ...
            for nnn in range(7,0,-1):
                if os.path.exists(Dir+'city_year212223/dataset_xview_'+str(year)+'_'+city+str(nnn)):
                    label_dir = Dir+'city_year212223/dataset_xview_'+str(year)+'_'+city+str(nnn)+'/labels/'
                    label_flg = 1
                    break

            if label_flg != 1:
                label_dir = Dir+'city_year212223/dataset_xview_'+str(year)+'_'+city+'/labels/'
        ...
        #np.savetxt('overall_object_cbg/'+i.split('/')[-1].split('.')[0]+'.txt',object_cnt, fmt = '%.5f')
            ...
            xview_cols = ['Fixed-wing aircraft','Passenger Vehicle','Building','Truck','Railway Vehicle','Maritime Vessel','Engineering Vehicle','Helipad','Vehicle Lot','Construction Site']
        #col_names = xview_cols + [x+'_conf' for x in xview_cols]
            pd_dict2.columns = xview_cols#names
        ...
        #col_names = DOTA_cols + [x+'_conf' for x in DOTA_cols]
            ...
            #print(len(pd_dict))
            pd_dict2.to_csv('../final_obd_city_xview/xview_'+city+'_'+str(year)+'.csv', index = False)
        #print(str(cnt)+' done')
...
#        subtask(str(i),Npara,Nlp,city_list)
