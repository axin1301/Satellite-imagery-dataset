...
def subtask(order,Npara,Nlp,shp_list,city_state_lut,state_osm_lut,multi_shp_year,typ):
    ...
    for k in (new_lines):
        ...
        drive_net = osm.get_pois()
        #drive_net.to_file('out_geojson_'+typ+'/' +city_name + '_'+typ+'_'+str(year)+'.geojson', driver='GeoJSON')
        #drive_net.to_file('out_geojson_2city/' +city_name + '_'+typ+'_'+str(year)+'.geojson', driver='GeoJSON')
        ...
        #drive_net = osm.get_landuse()
        #drive_net.to_file('Wichita_city_landuse.geojson', driver='GeoJSON') #print('1')
