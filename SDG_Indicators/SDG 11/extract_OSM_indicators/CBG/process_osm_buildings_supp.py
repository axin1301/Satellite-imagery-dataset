...
def subtask(order,Npara,Nlp,multi_shp_year,typ):
    ...
    for k in (new_lines):
        ...
        if os.path.exists(typ+'/'+city_name+'_cbg_'+typ+'_'+str(year)+'.csv'):
            continue
        ...
        city_name = shp_name.split('/')[-1].split('.')[0].split('_')[0]
        #print(city_name)
        attribute_shp = gpd.read_file('../../out_geojson_'+typ+'/'+city_name+'_'+typ+'_'+str(year)+'.geojson', driver = 'geojson')

        attr_within_cbg = gpd.sjoin(attribute_shp, shapefile,how = 'inner', op = 'intersects')
        attr_within_cbg.to_csv(typ+'/'+city_name+'_cbg_'+typ+'_'+str(year)+'.csv', index=False)
